#if (!require(maps, quietly=TRUE)) stop("You need to install the maps package!")
#if (!require(mapproj, quietly=TRUE)) stop("You need to install the mapproj package!")
#if (!require(maptools, quietly=TRUE)) stop("You need to install the maptools package!")
#
#
#grob_polylist <- function(coords, ...) {
...
#}
#
#grob_mappoly <- function (coords, gp=gpar(fill="grey80", col="white"), gphollow=gpar(fill="blue", col=NA), delta=100, projection="", ...) {
#	def <- function(x, default=1) if (is.null(x)) default else x
#
